...
const filteredData = communitiesData
  .filter((community) => community.village_name.toLowerCase().includes(searchTerm.toLowerCase()))
  .filter((community) => community.longitude !== null && community.latitude !== null);

const bounds = filteredData.reduce((acc, community) => {
  const [minLng, minLat] = acc;
...
map.fitBounds(bounds as mapboxgl.LngLatBoundsLike, { padding: 50 });
